...
def test(model_path, env, enforcer, test_runs, policy_frequency):
    ...
    Parameters:
        ...
        policy_frequency (float): The frequency at which the model takes actions (Hz).
        ...
        while not done and not truncated:
            ...
            if execute_enforcer:
                ...
                if x_front == float('inf'):
                    ...
                    # Keep the same action if the enforcer returns None
                    if enforced_action == None: 
                        logger.info("Enforcer maintained the same action (no changes applied).")
                        logger.info(f"Action: {action_descritpion}")
                    else:    
